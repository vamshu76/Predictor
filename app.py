...
with col3:
    ...
    if st.button('Predict'):
        st.markdown(f'<div style="background-color: #f0f0f5; padding: 10px; border: 1px solid gray;">'
                    f'<h4 style="color: white;">Compressive Strength (CS): {np.round(prediction_cs[0], 2)} Mpa</h4>'
                    f'<h4 style="color: white;">Tensile Strain (TS): {np.round(prediction_ts[0], 2)} %</h4>'
                    f'<h4 style="color: white;">Flexural Strength (FS): {np.round(prediction_fs[0], 2)} Mpa</h4>'
